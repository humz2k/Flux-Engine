...
void flux_scene_instantiate_prefab(enum fluxPrefabID prefab, fluxTransform transform){
    FLUX_ASSERT(n_objects < FLUX_MAX_GAMEOBJECTS, "FLUX<scene.c>: too many objects instantiated (%d >= %d)",n_objects, FLUX_MAX_GAMEOBJECTS);
    ...
    if (flux_gameobject_is_camera(obj) && (active_camera == NULL)){
        TraceLog(LOG_INFO,"FLUX<scene.c>: setting scene active camera to gameobject with id %d",flux_gameobject_get_id(obj));
        active_camera = obj;
    }
}
...
void flux_draw_scene(void){
    ...
    TraceLog(LOG_DEBUG,"FLUX<scene.c>: drawing scene with camera: position = {%g %g %g}, target = {%g %g %g}, up = {%g %g %g}, fovy = %g, projection = %d",
