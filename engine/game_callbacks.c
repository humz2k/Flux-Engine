...
static void add_callback(struct GameCallback* callbacks, void (*new_callback)(void)){
    FLUX_ASSERT((callbacks->n_callbacks < FLUX_MAX_GAME_CALLBACKS), "FLUX<game_callbacks.c>: reached max callbacks!");
...
void flux_init_game_callbacks(void){
    TraceLog(LOG_INFO,"FLUX<game_callbacks.c>: flux_init_game_callbacks called");
...
void flux_register_callback(enum FluxGameCallback event, void (*new_callback)(void)){
    switch (event){
        ...
        case fluxGameCallback_onGameClose:
            ...
            break;
        default:
            TraceLog(LOG_FATAL,"FLUX<game_callbacks.c>: unknown callback registered");
            break;
    }
}
