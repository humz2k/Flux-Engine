/**
 * @file gameobject.c
 * @brief Manages game objects including their properties, transformations, models, cameras, scripts, and children.
 *
 * Provides functions to allocate, initialize, and manipulate game objects within a scene. Supports setting models and cameras,
 * adding scripts and child objects, and accessing various properties of game objects.
 */
...
struct fluxGameObjectStruct {
    ...
    struct fluxTransform transform; ///< Transform of the game object in the scene.
    ...
    fluxScript scripts[FLUX_MAX_SCRIPTS]; ///< Scripts attached to the game object.
    int n_children; ///< Number of child objects.
    struct fluxGameObjectStruct* children[FLUX_MAX_CHILDREN]; ///< Child objects.
};
...
void flux_gameobject_set_model(fluxGameObject obj, Model model, Color tint){
    FLUX_ASSERT((obj != NULL),"FLUX<gameobject.c>: tried to set model of NULL game object!");
    FLUX_ASSERT(!(obj->is_camera),"FLUX<gameobject.c>: tried to add model to camera!");
    obj->has_model = 1;
    obj->model = model;
    obj->tint = tint;
}
...
void flux_gameobject_set_camera(fluxGameObject obj, float fovy, int projection){
    FLUX_ASSERT(obj != NULL,"FLUX<gameobject.c>: tried to set camera of NULL game object!");
    FLUX_ASSERT(!obj->has_model,"FLUX<gameobject.c>: tried to set camera of model!");
    obj->is_camera = 1;
    obj->cam.fovy = fovy;
...
void flux_gameobject_add_script(fluxGameObject obj, enum fluxScriptID script){
    FLUX_ASSERT(obj != NULL,"FLUX<gameobject.c>: tried to add script to NULL game object!");
    FLUX_ASSERT(obj->n_scripts < FLUX_MAX_SCRIPTS, "FLUX<gameobject.c>: game object already has FLUX_MAX_SCRIPTS scripts");
...
void flux_gameobject_add_child(fluxGameObject obj, fluxGameObject child){
    FLUX_ASSERT(obj != NULL,"FLUX<gameobject.c>: tried to add child to NULL game object!");
    FLUX_ASSERT(child != NULL, "FLUX<gameobject.c>: tried to add NULL child!");
    FLUX_ASSERT(obj->n_children < FLUX_MAX_CHILDREN,"FLUX<gameobject.c>: game object already has FLUX_MAX_CHILDREN scripts");
...
Camera3D flux_gameobject_get_raylib_camera(fluxGameObject obj){
    FLUX_ASSERT(flux_gameobject_is_camera(obj),"FLUX<gameobject.c>: tried to get camera of game object that isn't camera");
    Camera3D out;
...
void flux_gameobject_draw(fluxGameObject obj){
    ...
    if ((!flux_gameobject_is_camera(obj)) && (flux_gameobject_has_model(obj))){
        ...
        DrawModelEx(model,transform.pos,rot_axis,rot_amount,transform.scale,tint);
    }
}