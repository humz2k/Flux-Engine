...
static inline void draw_text_in_editor_rect(editorRect rect, Vector2 offset,
    ...
    float text_mul =
        text_width_mul < text_height_mul ? text_width_mul : text_height_mul;
    text_mul *= 0.8;
    Vector2 good_size =
        MeasureTextEx(editor_font, text, 20 * text_mul, 1 * text_mul);
    Vector2 pos = (Vector2){pix_rect.x + pix_rect.width * 0.5f,
                            pix_rect.y + pix_rect.height * 0.5f};
...
static inline void draw_text_in_editor_rect_left_aligned(editorRect rect,
                                                         Vector2 offset,
                                                         const char* text,
                                                         Color color) {
    Rectangle pix_rect = editor_rect_to_ray_rect(rect);
