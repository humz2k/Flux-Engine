...
void editor_init_input_boxes(void) {
    ...
    assert(allocated_text_input_boxes =
               (struct editorTextInputBoxStruct**)malloc(
                   sizeof(struct editorTextInputBoxStruct*) *
                   n_allocated_text_input_boxes));
}
...
static void grow_text_input_boxes(void) {
    TraceLog(LOG_FLUX_EDITOR,
             "growing allocated text_input_boxes from %d to %d",
             n_allocated_text_input_boxes, n_allocated_text_input_boxes * 2);
    n_allocated_text_input_boxes *= 2;
    assert(n_allocated_text_input_boxes > 0);
    assert(allocated_text_input_boxes =
               (struct editorTextInputBoxStruct**)realloc(
                   allocated_text_input_boxes,
                   sizeof(struct editorTextInputBoxStruct*) *
                       n_allocated_text_input_boxes));
}

editorTextInputBox
editor_make_text_input_box(editorRect rect, Color background_color,
                           Color edit_color, Color text_color,
                           void (*setText)(char*, editorTextInputBox),
                           void (*getText)(char*, editorTextInputBox)) {
    if (n_text_input_boxes >= n_allocated_text_input_boxes) {
        grow_text_input_boxes();
    }
    allocated_text_input_boxes[n_text_input_boxes] =
        malloc(sizeof(struct editorTextInputBoxStruct));
...
editorTextBox editor_make_text_box(editorRect rect, Color background_color,
                                   Color text_color,
                                   void (*getText)(char*, editorTextBox)) {
    if (n_text_input_boxes >= n_allocated_text_input_boxes) {
        grow_text_input_boxes();
    }
    allocated_text_input_boxes[n_text_input_boxes] =
...
bool editor_draw_text_input_box(editorTextInputBox box, Vector2 offset) {
    ...
    draw_text_in_editor_rect_left_aligned(box->rect, offset, box->current_input,
                                          box->text_color);
