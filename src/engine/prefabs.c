 ...
 *
 * This file provides the functionality to manage game prefabs, including
 * registration, instantiation, and cleanup of prefabs. It integrates with
 * raylib for rendering and supports game objects with models, cameras, scripts,
...
struct fluxPrefabStruct {
    ...
    enum fluxScriptID scripts[FLUX_MAX_SCRIPTS]; ///< Array of script IDs
                                                 ///< attached to the prefab.
    int n_children;                              ///< Number of child prefabs.
    enum fluxPrefabID
        children[FLUX_MAX_CHILDREN]; ///< Array of child prefab IDs.
};

static int n_prefabs = 0;              ///< Number of registered prefabs.
static int allocated_prefab_space = 0; ///< Allocated space for prefabs array.
static struct fluxPrefabStruct* allocated_prefabs =
    NULL; ///< Dynamic array of prefabs.

/**
 * Cleans up and deallocates memory used by prefabs.
 * This function should be called at the end of the program to ensure proper
 * memory management.
 */
void flux_close_prefabs(void) {
    TraceLog(LOG_INFO,
             "FLUX<prefabs.c>: flux_close_prefabs called (deleting %d prefabs)",
             n_prefabs);
 ...
 */
void flux_register_prefab(const char* name, const char* tag,
                          const char* model_path, int is_camera, int n_scripts,
                          enum fluxScriptID* scripts, int n_children,
                          enum fluxPrefabID* children) {
    FLUX_ASSERT((!(model_path && is_camera)),
                "FLUX<prefabs.c>: a prefab can't be a model and a camera");
    FLUX_ASSERT((n_scripts <= FLUX_MAX_SCRIPTS),
                "FLUX<prefabs.c>: tried to add too many scripts (%d > %d) to "
                "game object",
                n_scripts, FLUX_MAX_SCRIPTS);
    FLUX_ASSERT((n_children <= FLUX_MAX_CHILDREN),
                "FLUX<prefabs.c>: tried to add too many children (%d > %d) to "
                "game object",
                n_children, FLUX_MAX_CHILDREN);
    if (n_prefabs >= allocated_prefab_space) {
        allocated_prefab_space *= 2;
        assert(allocated_prefabs =
                   realloc(allocated_prefabs, sizeof(struct fluxPrefabStruct) *
                                                  allocated_prefab_space));
    }
    FLUX_ASSERT(n_prefabs < allocated_prefab_space,
                "FLUX<prefabs.c>: for some reason n_prefabs (%d) >= "
                "allocated_prefab_space (%d)",
                n_prefabs, allocated_prefab_space);
    ...
    for (int i = 0; i < n_children; i++) {
        FLUX_ASSERT(children[i] < n_prefabs,
                    "FLUX<prefabs.c>: prefabID %d is out of bounds (%d)",
                    (int)children[i], n_prefabs);
...
void flux_init_prefabs(void) {
    ...
    assert(allocated_prefabs = malloc(sizeof(struct fluxPrefabStruct) *
                                      allocated_prefab_space));
...
fluxGameObject flux_instantiate_prefab(enum fluxPrefabID prefabid,
    ...
    FLUX_ASSERT(prefabid < n_prefabs,
                "FLUX<prefabs.c>: prefabID %d is out of bounds (%d)", prefabid,
                n_prefabs);
    struct fluxPrefabStruct prefab = allocated_prefabs[prefabid];
    fluxGameObject out =
        flux_allocate_gameobject(prefab.name, prefab.tag, transform);
    ...
    for (int i = 0; i < prefab.n_children; i++) {
        flux_gameobject_add_child(
            out, flux_instantiate_prefab(prefab.children[i], transform));
    }
    return out;
}